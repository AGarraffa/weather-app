...
var stateEl = document.getElementById('states');
...
for (let i = 0; i < state.length; i++) {
    ...
    option.textContent = state[i].code;
    option.value = i+1; 
}
...
submitBtn.addEventListener("click", function(event) {
    ...
    // this is the syntax for getting the value of the state/country.
    // send this value to the geocoding value on button press
    console.log(stateEl[stateEl.selectedIndex].textContent);
    console.log(countryEl[countryEl.selectedIndex].textContent);
  });


    // use the dropdowns to build the weather api url. do a few if statements.
    // if state abrev = 'state' then don't include it 
    // if country != usa, hide the state dropdown


    // function for building the url based on the location data
    // function to pull the weather data
    // function to render the weather data (maybe use a defualt style so it can be used for all the cards)
    // save cities to local memory; append as buttons to be clicked 
